...
def quizemaker(qpool, index, altcodes):
    quiztmp2 = []

    for a in index:
        try:
            quiztmp1 = []
            quiztmp1.extend(list(qpool[a]))
            lr_alt = list(range(len(altcodes)))
            lr_quiztmp1 = list(range(len(quiztmp1[1])))
            lr = min(lr_alt,lr_quiztmp1)

            for x in lr_quiztmp1:
                if lr_quiztmp1 > lr_alt:
                    quiztmp1[1].pop()
                    lr_quiztmp1 = list(range(len(quiztmp1[1])))

            for b, c in zip(lr, altcodes):
                if ":" not in quiztmp1[1][b]:
                    quiztmp1[1][b] = c+": "+quiztmp1[1][b]
            quiztmp2.append(tuple(quiztmp1))

        except IndexError:
            print("Ignoring index %s - list index out of range"%(a))

        except TypeError:
            print("Ignoring index %s - unsupported operand type(s) for +: 'int' and 'str'"%(a))

    return quiztmp2


def gen_quiz(qpool, *index, altcodes=None, quiz=None):
    if not altcodes and not quiz:
        altcodes = 'ABCDEF'
        return (quizemaker(qpool, index, altcodes))

    if altcodes and not quiz:
            return (quizemaker(qpool, index, altcodes))
        
    if quiz and not altcodes:
        if not index:
            return quiz
        else:
            quiztmp2 = []
        ...
        quiz.extend((quizemaker(qpool, index, altcodes)))
        return quiz
